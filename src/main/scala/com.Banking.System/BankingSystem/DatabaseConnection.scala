...
object DatabaseConnection {
    ...
    def getTransactions(accountId: Int, count: Int): List[Transaction] = {
        ...
        while (resultSet.next) {
            ...
            val transaction = Transaction(Some(id), sourceAccount, destinationAccount, amount, date, TransactionType.fromString(transactionTypeString))
    ...
    def updateBalance(accountId: Int, amount: Double, updateOperator: String): Boolean = {
        val connection = getConnection

        val query = s"UPDATE accounts SET balance = balance $updateOperator ? WHERE id = ?"
        val preparedStatement = connection.prepareStatement(query)
